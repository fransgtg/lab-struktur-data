...
// Angka tidak boleh sama dalam baris, kolom, dan sub-kotak 3x3
        ...
        for (int col = 0; col < 9; col++) {
            if (board[row][col] == 0) { // Ketemu sel kosong
                for (int num = 1; num <= 9; num++) { // Coba masukkan angka dari 1 - 9
                    if (isValid(board, row, col, num)) { // Jika valid
                ...
                return false; // Jika tidak ada angka yg valid, backtrack
            }
        }
    }
    return true; // Jika tidak ada lagi sel kosong (solusi telah ditemukan) 
}
